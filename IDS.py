...
packetcsv = np.genfromtxt('packets.csv', delimiter=",")
packets = packetcsv[:,0]
stdevpack = np.std(packets)

#Standard Deviation of Bytes
bytecsv = np.genfromtxt('bytes.csv', delimiter=",")
bytes = bytecsv[:,0]
...
with open('ipsrc.csv', 'r') as t1, open('ipdst.csv', 'r') as t2:
    fileone = t1.readlines()
    filetwo = t2.readlines()

with open('flow.csv', 'w') as outFile:
    for line in fileone:
        if line not in filetwo:
            outFile.write(line) 
with open('flow.csv') as intflow:
...
with open('check.csv', 'w') as datafile:
